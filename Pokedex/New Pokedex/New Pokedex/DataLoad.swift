...
//        }
        ...
        for i in 1...list.count-2{
            let pokeVariables = list[i].components(separatedBy: ",")
...
//          poke.insert(Pokemon(number: number, name: name, type1: type1, type2: type2, total: total, hp: hp, attack: attack, defense: defense, sAttack: sAttack, sDefense: sDefense, speed: speed, generation: generation, legend: legend, image: image),at: i-1)
                ...
                if let imageData = data {
                    ...
                    poke.insert(Pokemon(number: number, name: name, typeOne: type1, typeTwo: type2, totalStats: total, health: hp, attack: attack, defense: defense, sAttack: sAttack, sDefense: sDefense, speed: speed, generation: generation, legendary: legend, pokeImage: image),at: i-1)
                    //print(name)
                }
            }
        return poke
